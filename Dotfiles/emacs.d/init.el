...
;; (~/src/wtf-wiki/bin/sync-emacs needs it).
;;
;; The guard only keeps *Warnings* clean: server-start on a name another emacs
;; already serves warns and leaves server-process nil, it does not take the
;; socket over. The failure worth guarding against is the opposite one --
;; whichever emacs owns the socket deletes it on the way out, and every emacs
;; that skipped server-start at startup is left serving nothing, with no
;; retry. A commit-message emacs (GIT_EDITOR, see Dotfiles/zshrc) that happens
;; to start first is enough to cause it, and sync-emacs then reports no server
;; and exits 0 forever. So re-check when idle and take the socket back.
(require 'server)

(defun wtf-claim-emacs-server ()
  "Serve emacsclient from this emacs unless another emacs already does.
Also preloads org-roam on a short idle timer, but only in the emacs that took
the socket: sync-emacs reaches that one and no other, and it needs org-roam
loaded to see the graph at all.  Preloading everywhere would run a full
org-roam-db-sync in throwaway processes emacsclient can never reach."
...
(use-package markdown-mode
  ...
  ;; `markdown-preview' builds standalone HTML and splices markdown-css-paths
  ;; in as <link> tags. github-markdown-css scopes every rule under
  ;; .markdown-body, so the body preamble/epilogue wrapper is required or the
  ;; stylesheet does nothing.
  ;;
  ;; --syntax-highlighting=none stops pandoc emitting its own <span> tokens,
  ...
  <style>
    ...
    .markdown-body { max-width: 860px; margin: 0 auto; padding: 2rem 2.5rem; }
    /* github-markdown-css already frames the block; keep the hljs theme off
       the padding and background so the two do not fight. */
    .markdown-body pre code.hljs { padding: 0; background: transparent; }
  </style>
  <script src=\""
                      (expand-file-name "highlight.min.js" user-emacs-directory)
                      "\"></script>
  <script src=\""
                      (expand-file-name "lisp.min.js" user-emacs-directory)
                      "\"></script>
  <script>
    document.addEventListener(\"DOMContentLoaded\", function () {
      // highlight.js has no Emacs Lisp grammar and registers the generic one
      // as \"lisp\" with no aliases, so point the elisp fence labels at it.
      hljs.registerAliases([\"elisp\", \"emacs-lisp\"], { languageName: \"lisp\" });
      // pandoc puts the language class on <pre>; hljs reads it from <code>.
      document.querySelectorAll(\"pre[class] > code:not([class])\").forEach(
        function (c) { c.className = c.parentNode.className; });
      hljs.highlightAll();
    });
  </script>"))
...
;; projectile-ripgrep builds the rg command as a single string and runs it
;; through `shell-file-name'. When that shell is zsh, zsh expands globs in the
;; command *before* rg launches. ripgrep.el's default excludes include the
;; unquoted pattern `--glob=!*.osc'; the `*' makes zsh treat the whole word as
;; a filename glob, and since nothing matches, zsh's `nomatch' option aborts
;; the entire command with "no matches found" — rg never runs, every search
;; returns nothing. bash leaves an unmatched glob as a literal string, so
;; running this one command under bash sidesteps the problem. Scoped to just
;; projectile-ripgrep so the interactive Emacs shell stays zsh.
(define-advice projectile-ripgrep
    (:around (orig-fn &rest args) use-bash-shell)
...
(defun wtf-org-roam-switch-graph (name)
  ...
  (let ((graph (wtf-org-roam-graph-paths name)))
    ...
    (org-roam-db-autosync-mode 1)
    ;; Enabling the mode installs global hooks and syncs, but the per-buffer
    ;; save-updater and completion functions hang off find-file-hook, so
    ;; buffers already visiting the graph we just switched *to* never got
    ;; them -- edits there would not reach the database. Arm them by hand.
    ;; (Disabling does strip them, from the graph we are leaving, because the
    ;; -1 above runs before the setq.)
...
May be empty -- a title with no alphanumerics leaves nothing to build from."
  (string-trim
...
(defun wtf-wiki-capture-target ()
  "Return the file the wiki node being captured should be created in.
A slug is not unique: \"C++\", \"C#\" and \"C\" all reduce to \"c\", and a
title with no alphanumerics reduces to nothing.  org-roam accepts both without
complaint -- file+head writes its header only when creating the file, so a
...
")
      :unnarrowed t)
     ;; wtf-wiki-capture-target returns an absolute path in the wiki, so this
     ;; files a page there whichever graph happens to be selected. See
     ;; ~/src/wtf-wiki/CLAUDE.md for the page format; the :ID: drawer is
     ;; org-roam's own doing.
     ("w" "wtf-wiki page" plain
      "%?"
      :target (file+head
               wtf-wiki-capture-target
               "#+title: ${title}\n#+filetags: :concept:\n\n")
      :unnarrowed t)))
  ...
  ;; org-roam-directory unbound and skips. wtf-claim-emacs-server preloads it
  ;; off the critical path, in the one emacs sync-emacs can actually reach.

  ;; Indexing a file means visiting it, and org-roam-with-file calls
  ;; hack-local-variables without binding enable-local-variables. The wiki
  ;; graph is written by tooling summarising external sources, so a page can
  ;; carry an `eval:' trailer it never meant to -- and the sync runs from a
  ;; timer, so the prompt arrives detached from any file the user opened.
  ;; Index without file-locals; org-roam forces org-mode anyway.
  (defun wtf-org-roam-without-local-variables (fn &rest args)
    "Apply FN to ARGS with file-local variables disabled."
  ...
  ;; Every capture command routes through org-roam-dailies--capture, but the
  ;; navigation commands do not: find-directory expands
  ;; org-roam-dailies-directory against org-roam-directory itself, and
  ;; goto-next-note reaches it through --daily-note-p and --list-files. Left
  ;; alone they would open the wiki's dailies directory, or refuse a note in
  ;; the notes graph as "not a daily-note". goto-previous-note needs no advice
  ;; of its own -- it delegates to goto-next-note.
  (dolist (fn '(org-roam-dailies--capture
                org-roam-dailies-find-directory
                org-roam-dailies-goto-next-note))
    (advice-add fn :around #'wtf-org-roam-dailies-in-notes-graph))
