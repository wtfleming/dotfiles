...
Two separate graphs, each with its own database — otherwise they would both use
=~/.emacs.d/org-roam.db=, and whichever synced last would overwrite the other's
index. =wiki= is the default; =C-c n s= switches.

Not every machine checks out both — the wiki is a separate repo, and some
machines will never have it. org-roam therefore starts on the first graph that
actually exists on disk, and leaves its database autosync off entirely if none
of them do.
...
** Presets
Can use a preset in a prompt by adding @preset-name to it. For example:
@explain println("hola");
Would run the explain preset and explain what the println call is
