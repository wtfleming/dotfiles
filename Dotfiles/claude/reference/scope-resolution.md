...
| `branch_base_sha` | where the branch under review begins: the merge base of `scope_head` with the base in use — `--base` when one was given, the default branch otherwise — resolved on **every** shape including the ones whose `base_sha` is null. It is what separates "already on the default branch" from "introduced by an earlier commit of this branch", which a scope narrower than the branch — a commit, a path, the working tree — cannot otherwise tell apart. Only the first is pre-existing. Two values mean the distinction cannot be drawn: **null**, when it could not be computed — no base ref resolved, a `scope_head` this clone does not have (ordinary on a fork PR whose head OID came from the API unfetched), or two histories with no common ancestor — and one **equal to `scope_head`**, where the head is already on the base and the merge base has collapsed onto it. Resolved independently of the scope's own base, so `base_ref`, `base_sha` and `default_branch_resolved` may all say "no base" while this field is set; a non-null `branch_base_sha` is the only gate on it. Costs no fetch of its own, and a stale ref only moves it backwards, widening what counts as the branch's own work |
| `base_stale`, `base_stale_reason` | the fetch failed; the merge base may be behind. `base_stale` is always a boolean, including on the shapes that consult no base — only `base_stale_reason` is null. To ask whether a base was consulted for **this scope**, read `base_ref == null` or `default_branch_resolved` — neither says anything about `branch_base_sha`, which resolves its own; `base_stale == false` means "no failed fetch", not "fetched and fresh" |
